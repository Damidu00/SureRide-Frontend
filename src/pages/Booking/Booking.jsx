...
export default function Booking(){
  ...
  const params = new URLSearchParams(location.search)
  const carId = params.get('carId')
  const daysParam = params.get('days')     // ✅ new
  const priceParam = params.get('price')   // ✅ new
  ...
  useEffect(()=>{
    ...
    // ✅ use price passed from CarDetails if available, else fallback
    if(priceParam){
  ...
  // recalc totalPrice only if user changes dates manually
  useEffect(()=>{
    if(!car || !startDate || !endDate) return
    // if priceParam exists, don't override
        ...
        {loading ? (
          ...
          <>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-semibold">{car.brand} {car.model}</div>
                    <div className="text-sm text-gray-600">{car.year} • {car.color}</div>
                    <div className="text-blue-600 font-bold mt-1">Rs: {car.pricePerDay}/day</div>
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="mb-4 text-lg font-semibold text-gray-800">
              Total to Pay: Rs: {totalPrice.toLocaleString()}
            </div>
